...
namespace linkedlist_quanly
{
    ...
    public class Post
    {
        ...
        public Post SharedFrom { get; set; } // Thêm thuộc tính để chỉ bài đăng gốc khi chia sẻ
        public string SharedBy { get; set; } // Người đã chia sẻ bài viết
        public List<Comment> Comments { get; set; } // Thêm danh sách bình luận
        public List<string> SharedByList { get; set; } // Danh sách những người chia sẻ bài viết
        ...
        public Post(string content, string mediaReference, string author)
        {
            ...
            Author = author;
            Next = null;
            SharedFrom = null;  // Không chia sẻ từ đâu khi mới tạo
            SharedBy = null;
            Comments = new List<Comment>(); // Khởi tạo danh sách bình luận
        }
        public void AddComment(string author, string content)
        {
            DateTime postTime = DateTime.Now;  // Lấy thời gian hiện tại
            Comments.Add(new Comment(author, content, postTime));  // Thêm comment với thời gian
        }



        public Post(string content, string mediaReference, string author, DateTime postTime, Post sharedFrom = null, string sharedBy = null)
        {
            Content = content;
            MediaReference = mediaReference;
            PostTime = postTime;
            Author = author;
            Next = null;
            SharedFrom = sharedFrom;  // Nếu là bài viết chia sẻ, SharedFrom sẽ không null
            SharedBy = sharedBy;
        }
    }
    ...
    public class SocialMediaLinkedList
    {
        ...
        public void SharePost(Post originalPost, string content, string mediaReference, string author, User currentUser)
        {
            // Kiểm tra nếu bài viết không phải là bài viết gốc
            if (originalPost != null)
            {
                // Tạo bài viết chia sẻ
                Post sharedPost = new Post(content, mediaReference, author, DateTime.Now, originalPost, currentUser.Username);

                // Thêm bài viết chia sẻ vào danh sách nếu người dùng là người chia sẻ
                if (author == currentUser.Username)
                {
                    AddPostToList(sharedPost);  // Chỉ thêm vào danh sách của người dùng
                }

                // Lưu bài viết chia sẻ vào hệ thống
                postManager.SavePost(sharedPost);
            }
        }
        ...
        public List<Post> GetAllPosts()
        {
            ...
            while (current != null)
            {
                // Nếu bài viết là một bài chia sẻ, lấy bài viết gốc
                if (current.SharedFrom != null)
                {
                    posts.Add(current.SharedFrom); // Thêm bài viết gốc
                }
                else
                {
                    posts.Add(current); // Thêm bài viết bình thường
                }
                current = current.Next;
            }
            return posts;
        ...
        public List<Post> GetUserPosts(string author)
        {
            ...
            while (current != null)
            {
                // Kiểm tra bài viết không phải là bài chia sẻ (SharedBy == null) và thuộc về user
                if (current.Author == author && current.SharedBy == null)
                {
